=begin
#eZmax API Definition (Full)

#This API expose all the functionnalities for the eZmax and eZsign applications.

The version of the OpenAPI document: 1.3.4
Contact: support-api@ezmax.ca
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'cgi'

module EzmaxApi
  class ObjectRejectedoffertopurchaseApi
    attr_accessor :api_client

    def initialize(api_client = ApiClient.default)
      @api_client = api_client
    end
    # Download multiples attachments from a Rejectedoffertopurchase
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param rejectedoffertopurchase_batch_download_v1_request [RejectedoffertopurchaseBatchDownloadV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [File]
    def rejectedoffertopurchase_batch_download_v1(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_batch_download_v1_request, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_batch_download_v1_with_http_info(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_batch_download_v1_request, opts)
      data
    end

    # Download multiples attachments from a Rejectedoffertopurchase
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param rejectedoffertopurchase_batch_download_v1_request [RejectedoffertopurchaseBatchDownloadV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(File, Integer, Hash)>] File data, response status code and response headers
    def rejectedoffertopurchase_batch_download_v1_with_http_info(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_batch_download_v1_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1, must be greater than or equal to 1.'
      end

      # verify the required parameter 'rejectedoffertopurchase_batch_download_v1_request' is set
      if @api_client.config.client_side_validation && rejectedoffertopurchase_batch_download_v1_request.nil?
        fail ArgumentError, "Missing the required parameter 'rejectedoffertopurchase_batch_download_v1_request' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1"
      end
      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/batchDownload'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/zip', 'text/xml', 'application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(rejectedoffertopurchase_batch_download_v1_request)

      # return_type
      return_type = opts[:debug_return_type] || 'File'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_batch_download_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_batch_download_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Rejectedoffertopurchase's attachments
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseGetAttachmentsV1Response]
    def rejectedoffertopurchase_get_attachments_v1(pki_rejectedoffertopurchase_id, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_attachments_v1_with_http_info(pki_rejectedoffertopurchase_id, opts)
      data
    end

    # Retrieve Rejectedoffertopurchase&#39;s attachments
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseGetAttachmentsV1Response, Integer, Hash)>] RejectedoffertopurchaseGetAttachmentsV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_attachments_v1_with_http_info(pki_rejectedoffertopurchase_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_attachments_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_attachments_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_attachments_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_attachments_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/getAttachments'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetAttachmentsV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_attachments_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_attachments_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Communication count
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseGetCommunicationCountV1Response]
    def rejectedoffertopurchase_get_communication_count_v1(pki_rejectedoffertopurchase_id, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_communication_count_v1_with_http_info(pki_rejectedoffertopurchase_id, opts)
      data
    end

    # Retrieve Communication count
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseGetCommunicationCountV1Response, Integer, Hash)>] RejectedoffertopurchaseGetCommunicationCountV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_communication_count_v1_with_http_info(pki_rejectedoffertopurchase_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_count_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_count_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_count_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_count_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/getCommunicationCount'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetCommunicationCountV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_count_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_communication_count_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Communication list
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseGetCommunicationListV1Response]
    def rejectedoffertopurchase_get_communication_list_v1(pki_rejectedoffertopurchase_id, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_communication_list_v1_with_http_info(pki_rejectedoffertopurchase_id, opts)
      data
    end

    # Retrieve Communication list
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseGetCommunicationListV1Response, Integer, Hash)>] RejectedoffertopurchaseGetCommunicationListV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_communication_list_v1_with_http_info(pki_rejectedoffertopurchase_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_list_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_list_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_list_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_list_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/getCommunicationList'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetCommunicationListV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communication_list_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_communication_list_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Rejectedoffertopurchase's Communicationrecipient
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseGetCommunicationrecipientsV1Response]
    def rejectedoffertopurchase_get_communicationrecipients_v1(pki_rejectedoffertopurchase_id, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_communicationrecipients_v1_with_http_info(pki_rejectedoffertopurchase_id, opts)
      data
    end

    # Retrieve Rejectedoffertopurchase&#39;s Communicationrecipient
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseGetCommunicationrecipientsV1Response, Integer, Hash)>] RejectedoffertopurchaseGetCommunicationrecipientsV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_communicationrecipients_v1_with_http_info(pki_rejectedoffertopurchase_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationrecipients_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationrecipients_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationrecipients_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationrecipients_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/getCommunicationrecipients'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetCommunicationrecipientsV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationrecipients_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_communicationrecipients_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Rejectedoffertopurchase's Communicationsender
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseGetCommunicationsendersV1Response]
    def rejectedoffertopurchase_get_communicationsenders_v1(pki_rejectedoffertopurchase_id, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_communicationsenders_v1_with_http_info(pki_rejectedoffertopurchase_id, opts)
      data
    end

    # Retrieve Rejectedoffertopurchase&#39;s Communicationsender
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseGetCommunicationsendersV1Response, Integer, Hash)>] RejectedoffertopurchaseGetCommunicationsendersV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_communicationsenders_v1_with_http_info(pki_rejectedoffertopurchase_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationsenders_v1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationsenders_v1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationsenders_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationsenders_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/getCommunicationsenders'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetCommunicationsendersV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_communicationsenders_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_communicationsenders_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Rejectedoffertopurchase list
    # 
    # @param [Hash] opts the optional parameters
    # @option opts [String] :e_order_by Specify how you want the results to be sorted
    # @option opts [Integer] :i_row_max 
    # @option opts [Integer] :i_row_offset  (default to 0)
    # @option opts [HeaderAcceptLanguage] :accept_language 
    # @option opts [String] :s_filter 
    # @return [RejectedoffertopurchaseGetListV1Response]
    def rejectedoffertopurchase_get_list_v1(opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_get_list_v1_with_http_info(opts)
      data
    end

    # Retrieve Rejectedoffertopurchase list
    # 
    # @param [Hash] opts the optional parameters
    # @option opts [String] :e_order_by Specify how you want the results to be sorted
    # @option opts [Integer] :i_row_max 
    # @option opts [Integer] :i_row_offset  (default to 0)
    # @option opts [HeaderAcceptLanguage] :accept_language 
    # @option opts [String] :s_filter 
    # @return [Array<(RejectedoffertopurchaseGetListV1Response, Integer, Hash)>] RejectedoffertopurchaseGetListV1Response data, response status code and response headers
    def rejectedoffertopurchase_get_list_v1_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_list_v1 ...'
      end
      allowable_values = ["pkiRejectedoffertopurchaseID_ASC", "pkiRejectedoffertopurchaseID_DESC", "sRejectedoffertopurchaseNumber_ASC", "sRejectedoffertopurchaseNumber_DESC", "dtRejectedoffertopurchaseDate_ASC", "dtRejectedoffertopurchaseDate_DESC", "bRejectedoffertopurchaseIsactive_ASC", "bRejectedoffertopurchaseIsactive_DESC", "bRejectedoffertopurchaseLinkedtoinscription_ASC", "bRejectedoffertopurchaseLinkedtoinscription_DESC", "dtCreatedDate_ASC", "dtCreatedDate_DESC", "sAddressCivic_ASC", "sAddressCivic_DESC", "sAddressStreet_ASC", "sAddressStreet_DESC", "sAddressSuite_ASC", "sAddressSuite_DESC", "sAddressCity_ASC", "sAddressCity_DESC", "sAddressZip_ASC", "sAddressZip_DESC", "sProvinceNameX_ASC", "sProvinceNameX_DESC", "sCountryNameX_ASC", "sCountryNameX_DESC"]
      if @api_client.config.client_side_validation && opts[:'e_order_by'] && !allowable_values.include?(opts[:'e_order_by'])
        fail ArgumentError, "invalid value for \"e_order_by\", must be one of #{allowable_values}"
      end
      if @api_client.config.client_side_validation && !opts[:'i_row_max'].nil? && opts[:'i_row_max'] > 10000
        fail ArgumentError, 'invalid value for "opts[:"i_row_max"]" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_list_v1, must be smaller than or equal to 10000.'
      end

      if @api_client.config.client_side_validation && !opts[:'i_row_max'].nil? && opts[:'i_row_max'] < 1
        fail ArgumentError, 'invalid value for "opts[:"i_row_max"]" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_list_v1, must be greater than or equal to 1.'
      end

      if @api_client.config.client_side_validation && !opts[:'i_row_offset'].nil? && opts[:'i_row_offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"i_row_offset"]" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_list_v1, must be greater than or equal to 0.'
      end

      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/getList'

      # query parameters
      query_params = opts[:query_params] || {}
      query_params[:'eOrderBy'] = opts[:'e_order_by'] if !opts[:'e_order_by'].nil?
      query_params[:'iRowMax'] = opts[:'i_row_max'] if !opts[:'i_row_max'].nil?
      query_params[:'iRowOffset'] = opts[:'i_row_offset'] if !opts[:'i_row_offset'].nil?
      query_params[:'sFilter'] = opts[:'s_filter'] if !opts[:'s_filter'].nil?

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']) unless header_params['Accept']
      header_params[:'Accept-Language'] = opts[:'accept_language'] if !opts[:'accept_language'].nil?

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseGetListV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_get_list_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_get_list_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Import attachments into the Rejectedoffertopurchase
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param rejectedoffertopurchase_import_into_edmv1_request [RejectedoffertopurchaseImportIntoEDMV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [RejectedoffertopurchaseImportIntoEDMV1Response]
    def rejectedoffertopurchase_import_into_edmv1(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_import_into_edmv1_request, opts = {})
      data, _status_code, _headers = rejectedoffertopurchase_import_into_edmv1_with_http_info(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_import_into_edmv1_request, opts)
      data
    end

    # Import attachments into the Rejectedoffertopurchase
    # 
    # @param pki_rejectedoffertopurchase_id [Integer] 
    # @param rejectedoffertopurchase_import_into_edmv1_request [RejectedoffertopurchaseImportIntoEDMV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(RejectedoffertopurchaseImportIntoEDMV1Response, Integer, Hash)>] RejectedoffertopurchaseImportIntoEDMV1Response data, response status code and response headers
    def rejectedoffertopurchase_import_into_edmv1_with_http_info(pki_rejectedoffertopurchase_id, rejectedoffertopurchase_import_into_edmv1_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1 ...'
      end
      # verify the required parameter 'pki_rejectedoffertopurchase_id' is set
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_rejectedoffertopurchase_id' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1"
      end
      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id > 65535
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_rejectedoffertopurchase_id < 1
        fail ArgumentError, 'invalid value for "pki_rejectedoffertopurchase_id" when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1, must be greater than or equal to 1.'
      end

      # verify the required parameter 'rejectedoffertopurchase_import_into_edmv1_request' is set
      if @api_client.config.client_side_validation && rejectedoffertopurchase_import_into_edmv1_request.nil?
        fail ArgumentError, "Missing the required parameter 'rejectedoffertopurchase_import_into_edmv1_request' when calling ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1"
      end
      # resource path
      local_var_path = '/1/object/rejectedoffertopurchase/{pkiRejectedoffertopurchaseID}/importIntoEDM'.sub('{pkiRejectedoffertopurchaseID}', CGI.escape(pki_rejectedoffertopurchase_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(rejectedoffertopurchase_import_into_edmv1_request)

      # return_type
      return_type = opts[:debug_return_type] || 'RejectedoffertopurchaseImportIntoEDMV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectRejectedoffertopurchaseApi.rejectedoffertopurchase_import_into_edmv1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectRejectedoffertopurchaseApi#rejectedoffertopurchase_import_into_edmv1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
end
