=begin
#eZmax API Definition (Full)

#This API expose all the functionnalities for the eZmax and eZsign applications.

The version of the OpenAPI document: 1.3.4
Contact: support-api@ezmax.ca
Generated by: https://openapi-generator.tech
Generator version: 7.25.0

=end

require 'cgi'

module EzmaxApi
  class ObjectBuyercontractApi
    attr_accessor :api_client

    def initialize(api_client = ApiClient.default)
      @api_client = api_client
    end
    # Download multiples attachments from a Buyercontract
    # @param pki_buyercontract_id [Integer] 
    # @param buyercontract_batch_download_v1_request [BuyercontractBatchDownloadV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [File]
    def buyercontract_batch_download_v1(pki_buyercontract_id, buyercontract_batch_download_v1_request, opts = {})
      data, _status_code, _headers = buyercontract_batch_download_v1_with_http_info(pki_buyercontract_id, buyercontract_batch_download_v1_request, opts)
      data
    end

    # Download multiples attachments from a Buyercontract
    # @param pki_buyercontract_id [Integer] 
    # @param buyercontract_batch_download_v1_request [BuyercontractBatchDownloadV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(File, Integer, Hash)>] File data, response status code and response headers
    def buyercontract_batch_download_v1_with_http_info(pki_buyercontract_id, buyercontract_batch_download_v1_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_batch_download_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_batch_download_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_batch_download_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_batch_download_v1, must be greater than or equal to 1.'
      end

      # verify the required parameter 'buyercontract_batch_download_v1_request' is set
      if @api_client.config.client_side_validation && buyercontract_batch_download_v1_request.nil?
        fail ArgumentError, "Missing the required parameter 'buyercontract_batch_download_v1_request' when calling ObjectBuyercontractApi.buyercontract_batch_download_v1"
      end
      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/batchDownload'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/zip', 'text/xml', 'application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(buyercontract_batch_download_v1_request)

      # return_type
      return_type = opts[:debug_return_type] || 'File'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_batch_download_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_batch_download_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Buyercontract's attachments
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractGetAttachmentsV1Response]
    def buyercontract_get_attachments_v1(pki_buyercontract_id, opts = {})
      data, _status_code, _headers = buyercontract_get_attachments_v1_with_http_info(pki_buyercontract_id, opts)
      data
    end

    # Retrieve Buyercontract&#39;s attachments
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractGetAttachmentsV1Response, Integer, Hash)>] BuyercontractGetAttachmentsV1Response data, response status code and response headers
    def buyercontract_get_attachments_v1_with_http_info(pki_buyercontract_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_attachments_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_get_attachments_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_attachments_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_attachments_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/getAttachments'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetAttachmentsV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_attachments_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_attachments_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Communication count
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractGetCommunicationCountV1Response]
    def buyercontract_get_communication_count_v1(pki_buyercontract_id, opts = {})
      data, _status_code, _headers = buyercontract_get_communication_count_v1_with_http_info(pki_buyercontract_id, opts)
      data
    end

    # Retrieve Communication count
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractGetCommunicationCountV1Response, Integer, Hash)>] BuyercontractGetCommunicationCountV1Response data, response status code and response headers
    def buyercontract_get_communication_count_v1_with_http_info(pki_buyercontract_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_communication_count_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_get_communication_count_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communication_count_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communication_count_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/getCommunicationCount'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetCommunicationCountV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_communication_count_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_communication_count_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Communication list
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractGetCommunicationListV1Response]
    def buyercontract_get_communication_list_v1(pki_buyercontract_id, opts = {})
      data, _status_code, _headers = buyercontract_get_communication_list_v1_with_http_info(pki_buyercontract_id, opts)
      data
    end

    # Retrieve Communication list
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractGetCommunicationListV1Response, Integer, Hash)>] BuyercontractGetCommunicationListV1Response data, response status code and response headers
    def buyercontract_get_communication_list_v1_with_http_info(pki_buyercontract_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_communication_list_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_get_communication_list_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communication_list_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communication_list_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/getCommunicationList'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetCommunicationListV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_communication_list_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_communication_list_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Buyercontract's Communicationrecipient
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractGetCommunicationrecipientsV1Response]
    def buyercontract_get_communicationrecipients_v1(pki_buyercontract_id, opts = {})
      data, _status_code, _headers = buyercontract_get_communicationrecipients_v1_with_http_info(pki_buyercontract_id, opts)
      data
    end

    # Retrieve Buyercontract&#39;s Communicationrecipient
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractGetCommunicationrecipientsV1Response, Integer, Hash)>] BuyercontractGetCommunicationrecipientsV1Response data, response status code and response headers
    def buyercontract_get_communicationrecipients_v1_with_http_info(pki_buyercontract_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_communicationrecipients_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_get_communicationrecipients_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communicationrecipients_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communicationrecipients_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/getCommunicationrecipients'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetCommunicationrecipientsV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_communicationrecipients_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_communicationrecipients_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Buyercontract's Communicationsender
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractGetCommunicationsendersV1Response]
    def buyercontract_get_communicationsenders_v1(pki_buyercontract_id, opts = {})
      data, _status_code, _headers = buyercontract_get_communicationsenders_v1_with_http_info(pki_buyercontract_id, opts)
      data
    end

    # Retrieve Buyercontract&#39;s Communicationsender
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractGetCommunicationsendersV1Response, Integer, Hash)>] BuyercontractGetCommunicationsendersV1Response data, response status code and response headers
    def buyercontract_get_communicationsenders_v1_with_http_info(pki_buyercontract_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_communicationsenders_v1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_get_communicationsenders_v1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communicationsenders_v1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_get_communicationsenders_v1, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/getCommunicationsenders'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetCommunicationsendersV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_communicationsenders_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_communicationsenders_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Retrieve Buyercontract list
    # Enum values that can be filtered in query parameter *sFilter*:  | Variable | Valid values | |---|---| | eBuyercontractStep | Imported<br>Active<br>Modified<br>ContractEnded<br>ExpiredContract<br>Bought | | eBuyercontractType | Rent<br>Sale<br>RentOrSale |
    # @param [Hash] opts the optional parameters
    # @option opts [String] :e_order_by Specify how you want the results to be sorted
    # @option opts [Integer] :i_row_max 
    # @option opts [Integer] :i_row_offset  (default to 0)
    # @option opts [HeaderAcceptLanguage] :accept_language 
    # @option opts [String] :s_filter 
    # @return [BuyercontractGetListV1Response]
    def buyercontract_get_list_v1(opts = {})
      data, _status_code, _headers = buyercontract_get_list_v1_with_http_info(opts)
      data
    end

    # Retrieve Buyercontract list
    # Enum values that can be filtered in query parameter *sFilter*:  | Variable | Valid values | |---|---| | eBuyercontractStep | Imported&lt;br&gt;Active&lt;br&gt;Modified&lt;br&gt;ContractEnded&lt;br&gt;ExpiredContract&lt;br&gt;Bought | | eBuyercontractType | Rent&lt;br&gt;Sale&lt;br&gt;RentOrSale |
    # @param [Hash] opts the optional parameters
    # @option opts [String] :e_order_by Specify how you want the results to be sorted
    # @option opts [Integer] :i_row_max 
    # @option opts [Integer] :i_row_offset  (default to 0)
    # @option opts [HeaderAcceptLanguage] :accept_language 
    # @option opts [String] :s_filter 
    # @return [Array<(BuyercontractGetListV1Response, Integer, Hash)>] BuyercontractGetListV1Response data, response status code and response headers
    def buyercontract_get_list_v1_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_get_list_v1 ...'
      end
      allowable_values = ["pkiBuyercontractID_ASC", "pkiBuyercontractID_DESC", "fkiInscriptiontypeID_ASC", "fkiInscriptiontypeID_DESC", "sInscriptiontypeNameX_ASC", "sInscriptiontypeNameX_DESC", "eBuyercontractStep_ASC", "eBuyercontractStep_DESC", "dBuyercontractMinimumprice_ASC", "dBuyercontractMinimumprice_DESC", "dBuyercontractMaximumprice_ASC", "dBuyercontractMaximumprice_DESC", "eBuyercontractType_ASC", "eBuyercontractType_DESC", "dtBuyercontractDate_ASC", "dtBuyercontractDate_DESC", "dtBuyercontractExpirationdate_ASC", "dtBuyercontractExpirationdate_DESC", "bBuyercontractIsactive_ASC", "bBuyercontractIsactive_DESC"]
      if @api_client.config.client_side_validation && opts[:'e_order_by'] && !allowable_values.include?(opts[:'e_order_by'])
        fail ArgumentError, "invalid value for \"e_order_by\", must be one of #{allowable_values}"
      end
      if @api_client.config.client_side_validation && !opts[:'i_row_max'].nil? && opts[:'i_row_max'] > 10000
        fail ArgumentError, 'invalid value for "opts[:"i_row_max"]" when calling ObjectBuyercontractApi.buyercontract_get_list_v1, must be smaller than or equal to 10000.'
      end

      if @api_client.config.client_side_validation && !opts[:'i_row_max'].nil? && opts[:'i_row_max'] < 1
        fail ArgumentError, 'invalid value for "opts[:"i_row_max"]" when calling ObjectBuyercontractApi.buyercontract_get_list_v1, must be greater than or equal to 1.'
      end

      if @api_client.config.client_side_validation && !opts[:'i_row_offset'].nil? && opts[:'i_row_offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"i_row_offset"]" when calling ObjectBuyercontractApi.buyercontract_get_list_v1, must be greater than or equal to 0.'
      end

      # resource path
      local_var_path = '/1/object/buyercontract/getList'

      # query parameters
      query_params = opts[:query_params] || {}
      query_params[:'eOrderBy'] = opts[:'e_order_by'] if !opts[:'e_order_by'].nil?
      query_params[:'iRowMax'] = opts[:'i_row_max'] if !opts[:'i_row_max'].nil?
      query_params[:'iRowOffset'] = opts[:'i_row_offset'] if !opts[:'i_row_offset'].nil?
      query_params[:'sFilter'] = opts[:'s_filter'] if !opts[:'s_filter'].nil?

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']) unless header_params['Accept']
      header_params[:'Accept-Language'] = opts[:'accept_language'] if !opts[:'accept_language'].nil?

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body]

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractGetListV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_get_list_v1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_get_list_v1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end

    # Import attachments into the Buyercontract
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param buyercontract_import_into_edmv1_request [BuyercontractImportIntoEDMV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [BuyercontractImportIntoEDMV1Response]
    def buyercontract_import_into_edmv1(pki_buyercontract_id, buyercontract_import_into_edmv1_request, opts = {})
      data, _status_code, _headers = buyercontract_import_into_edmv1_with_http_info(pki_buyercontract_id, buyercontract_import_into_edmv1_request, opts)
      data
    end

    # Import attachments into the Buyercontract
    # 
    # @param pki_buyercontract_id [Integer] 
    # @param buyercontract_import_into_edmv1_request [BuyercontractImportIntoEDMV1Request] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(BuyercontractImportIntoEDMV1Response, Integer, Hash)>] BuyercontractImportIntoEDMV1Response data, response status code and response headers
    def buyercontract_import_into_edmv1_with_http_info(pki_buyercontract_id, buyercontract_import_into_edmv1_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectBuyercontractApi.buyercontract_import_into_edmv1 ...'
      end
      # verify the required parameter 'pki_buyercontract_id' is set
      if @api_client.config.client_side_validation && pki_buyercontract_id.nil?
        fail ArgumentError, "Missing the required parameter 'pki_buyercontract_id' when calling ObjectBuyercontractApi.buyercontract_import_into_edmv1"
      end
      if @api_client.config.client_side_validation && pki_buyercontract_id > 65535
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_import_into_edmv1, must be smaller than or equal to 65535.'
      end

      if @api_client.config.client_side_validation && pki_buyercontract_id < 1
        fail ArgumentError, 'invalid value for "pki_buyercontract_id" when calling ObjectBuyercontractApi.buyercontract_import_into_edmv1, must be greater than or equal to 1.'
      end

      # verify the required parameter 'buyercontract_import_into_edmv1_request' is set
      if @api_client.config.client_side_validation && buyercontract_import_into_edmv1_request.nil?
        fail ArgumentError, "Missing the required parameter 'buyercontract_import_into_edmv1_request' when calling ObjectBuyercontractApi.buyercontract_import_into_edmv1"
      end
      # resource path
      local_var_path = '/1/object/buyercontract/{pkiBuyercontractID}/importIntoEDM'.sub('{pkiBuyercontractID}', CGI.escape(pki_buyercontract_id.to_s))

      # query parameters
      query_params = opts[:query_params] || {}

      # header parameters
      header_params = opts[:header_params] || {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json']) unless header_params['Accept']
      # HTTP header 'Content-Type'
      content_type = @api_client.select_header_content_type(['application/json'])
      if !content_type.nil?
          header_params['Content-Type'] = content_type
      end

      # form parameters
      form_params = opts[:form_params] || {}

      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(buyercontract_import_into_edmv1_request)

      # return_type
      return_type = opts[:debug_return_type] || 'BuyercontractImportIntoEDMV1Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectBuyercontractApi.buyercontract_import_into_edmv1",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => return_type
      )

      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectBuyercontractApi#buyercontract_import_into_edmv1\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
end
