...
module EzmaxApi
  class ObjectFranchiseofficeApi
    ...
    def franchiseoffice_get_autocomplete_v2(s_selector, opts = {})
      data, _status_code, _headers = franchiseoffice_get_autocomplete_v2_with_http_info(s_selector, opts)
      data
    end
    ...
    # @option opts [String] :s_query Allow to filter the returned results
    # @option opts [HeaderAcceptLanguage] :accept_language 
    # @return [Array<(FranchiseofficeGetAutocompleteV2Response, Integer, Hash)>] FranchiseofficeGetAutocompleteV2Response data, response status code and response headers
    def franchiseoffice_get_autocomplete_v2_with_http_info(s_selector, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ObjectFranchiseofficeApi.franchiseoffice_get_autocomplete_v2 ...'
      end
      # verify the required parameter 's_selector' is set
      if @api_client.config.client_side_validation && s_selector.nil?
        fail ArgumentError, "Missing the required parameter 's_selector' when calling ObjectFranchiseofficeApi.franchiseoffice_get_autocomplete_v2"
      end
      # verify enum value
      ...
      # resource path
      local_var_path = '/2/object/franchiseoffice/getAutocomplete/{sSelector}'.sub('{' + 'sSelector' + '}', CGI.escape(s_selector.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'FranchiseofficeGetAutocompleteV2Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['Authorization']

      new_options = opts.merge(
        :operation => :"ObjectFranchiseofficeApi.franchiseoffice_get_autocomplete_v2",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ObjectFranchiseofficeApi#franchiseoffice_get_autocomplete_v2\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
