  ...
  class CommunicationResponseCompound
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::CommunicationResponseCompound` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::CommunicationResponseCompound`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pki_communication_id, e_communication_importance, e_communication_type, s_communication_subject, s_communication_bodyurl, e_communication_direction, i_communicationrecipient_count, b_communication_private, obj_descriptionstatic_sender, obj_emailstatic_sender, obj_phonestatic_sender, obj_audit, a_obj_communicationattachment, a_obj_communicationrecipient, a_obj_communicationexternalrecipient].hash
    end
