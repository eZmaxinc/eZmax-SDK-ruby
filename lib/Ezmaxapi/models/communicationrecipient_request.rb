  ...
  class CommunicationrecipientRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::CommunicationrecipientRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::CommunicationrecipientRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pki_communicationrecipient_id, fki_agent_id, fki_agentincorporation_id, fki_broker_id, fki_customer_id, fki_employee_id, fki_assistant_id, fki_externalbroker_id, fki_ezsignsigner_id, fki_notary_id, fki_supplier_id, fki_user_id, e_communicationrecipient_type].hash
    end
