...
module EzmaxApi
  ...
  class PermissionRequestCompound < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::PermissionRequestCompound` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::PermissionRequestCompound`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@pki_permission_id.nil? && @pki_permission_id > 65535
      return false if !@pki_permission_id.nil? && @pki_permission_id < 0
    ...
    def hash
      [pki_permission_id, fki_user_id, fki_apikey_id, fki_usergroup_id, fki_company_id, fki_modulesection_id].hash
    end
