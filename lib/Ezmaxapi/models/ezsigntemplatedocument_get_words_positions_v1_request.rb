...
module EzmaxApi
  ...
  class EzsigntemplatedocumentGetWordsPositionsV1Request < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::EzsigntemplatedocumentGetWordsPositionsV1Request` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::EzsigntemplatedocumentGetWordsPositionsV1Request`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'b_word_case_sensitive')
        ...
        self.b_word_case_sensitive = nil
      end
    ...
    # @param [Object] b_word_case_sensitive Value to be assigned
    def b_word_case_sensitive=(b_word_case_sensitive)
      if b_word_case_sensitive.nil?
        fail ArgumentError, 'b_word_case_sensitive cannot be nil'
      end

      @b_word_case_sensitive = b_word_case_sensitive
    end
