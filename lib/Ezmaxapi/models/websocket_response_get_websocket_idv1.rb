...
module EzmaxApi
  ...
  class WebsocketResponseGetWebsocketIDV1 < ApiModelBase
    # The Type of message
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::WebsocketResponseGetWebsocketIDV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::WebsocketResponseGetWebsocketIDV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      e_websocket_messagetype_validator = EnumAttributeValidator.new('String', ["Response-GetWebsocketID-V1"])
    ...
    def e_websocket_messagetype=(e_websocket_messagetype)
      validator = EnumAttributeValidator.new('String', ["Response-GetWebsocketID-V1"])
    ...
    def hash
      [e_websocket_messagetype, m_payload].hash
    end
