...
module EzmaxApi
  ...
  class CustomDnsrecordResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::CustomDnsrecordResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] s_dnsrecord_name Value to be assigned
    def s_dnsrecord_name=(s_dnsrecord_name)
      if s_dnsrecord_name.nil?
        fail ArgumentError, 's_dnsrecord_name cannot be nil'
      end

      @s_dnsrecord_name = s_dnsrecord_name
    end

    # Custom attribute writer method with validation
    # @param [Object] b_dnsrecord_must_match Value to be assigned
    def b_dnsrecord_must_match=(b_dnsrecord_must_match)
      if b_dnsrecord_must_match.nil?
        fail ArgumentError, 'b_dnsrecord_must_match cannot be nil'
      end

      @b_dnsrecord_must_match = b_dnsrecord_must_match
    end
