...
module EzmaxApi
  ...
  class AddressResponseCompound < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::AddressResponseCompound` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::AddressResponseCompound`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @pki_address_id.nil?
        invalid_properties.push('invalid value for "pki_address_id", pki_address_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @pki_address_id.nil?
      return false if @pki_address_id < 0
    ...
    def hash
      [pki_address_id, fki_addresstype_id, s_address_civic, s_address_street, s_address_suite, s_address_city, fki_province_id, s_province_name_x, fki_country_id, s_country_name_x, s_address_zip, f_address_longitude, f_address_latitude].hash
    end
