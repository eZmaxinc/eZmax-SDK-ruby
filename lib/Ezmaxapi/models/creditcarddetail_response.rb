  ...
  class CreditcarddetailResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::CreditcarddetailResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::CreditcarddetailResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @s_creditcarddetail_civic !~ Regexp.new(/^.{0,8}$/)
      return false if @s_creditcarddetail_street.nil?
      return false if @s_creditcarddetail_street !~ Regexp.new(/^.{0,40}$/)
      return false if @s_creditcarddetail_zip.nil?
      return false if @s_creditcarddetail_zip !~ Regexp.new(/^.{0,10}$/)
      true
    end
    ...
    def hash
      [pki_creditcarddetail_id, fki_creditcardtype_id, s_creditcarddetail_numbermasked, i_creditcarddetail_expirationmonth, i_creditcarddetail_expirationyear, s_creditcarddetail_civic, s_creditcarddetail_street, s_creditcarddetail_zip].hash
    end
