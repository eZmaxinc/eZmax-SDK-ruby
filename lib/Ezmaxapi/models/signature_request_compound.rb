  ...
  class SignatureRequestCompound
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::SignatureRequestCompound` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::SignatureRequestCompound`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @t_signature_svg.nil?
        invalid_properties.push('invalid value for "t_signature_svg", t_signature_svg cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @t_signature_svg.nil?
      return false if @t_signature_svg !~ Regexp.new(/^.{0,32767}$/)
      true
    end
    ...
    def hash
      [pki_signature_id, t_signature_svg].hash
    end
