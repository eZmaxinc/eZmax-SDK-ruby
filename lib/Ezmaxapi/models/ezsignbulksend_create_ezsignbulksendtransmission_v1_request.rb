  ...
  class EzsignbulksendCreateEzsignbulksendtransmissionV1Request
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::EzsignbulksendCreateEzsignbulksendtransmissionV1Request` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::EzsignbulksendCreateEzsignbulksendtransmissionV1Request`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [fki_userlogintype_id, fki_ezsigntsarequirement_id, s_ezsignbulksendtransmission_description, dt_ezsigndocument_duedate, e_ezsignfolder_sendreminderfrequency, t_extra_message, s_csv_base64].hash
    end
