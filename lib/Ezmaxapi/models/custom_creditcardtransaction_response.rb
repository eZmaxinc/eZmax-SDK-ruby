...
module EzmaxApi
  ...
  class CustomCreditcardtransactionResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::CustomCreditcardtransactionResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^(([X]{4}[ ]){3}(\d){4})|([X]{4}[ ][X]{6}[ ][X][\d]{4})$/)
    ...
    def valid?
      ...
      return false if @s_creditcardtransaction_partiallydecryptednumber !~ Regexp.new(/^(([X]{4}[ ]){3}(\d){4})|([X]{4}[ ][X]{6}[ ][X][\d]{4})$/)
