...
module EzmaxApi
  ...
  class EmailResponseCompound < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::EmailResponseCompound` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::EmailResponseCompound`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @pki_email_id.nil?
        invalid_properties.push('invalid value for "pki_email_id", pki_email_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @pki_email_id.nil?
      return false if @pki_email_id > 16777215
      return false if @pki_email_id < 1
