...
module EzmaxApi
  ...
  class ApikeyResponseCompound < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::ApikeyResponseCompound` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::ApikeyResponseCompound`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pki_apikey_id, fki_user_id, fki_ezmaxpartnerproductstage_id, obj_apikey_description, obj_contact_name, s_apikey_apikey, s_apikey_secret, b_apikey_isactive, b_apikey_issigned, obj_audit].hash
    end
