...
module EzmaxApi
  # A Discussion Object
  class DiscussionRequestPatch < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::DiscussionRequestPatch` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::DiscussionRequestPatch`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@s_discussion_description.nil? && @s_discussion_description !~ pattern
    ...
    def valid?
      ...
      return false if !@s_discussion_description.nil? && @s_discussion_description !~ Regexp.new(/^.{0,75}$/)
      true
    end
    ...
    def hash
      [s_discussion_description, b_discussion_closed].hash
    end
