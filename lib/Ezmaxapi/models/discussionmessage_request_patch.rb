...
module EzmaxApi
  ...
  class DiscussionmessageRequestPatch < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::DiscussionmessageRequestPatch` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::DiscussionmessageRequestPatch`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@t_discussionmessage_content.nil? && @t_discussionmessage_content !~ pattern
    ...
    def valid?
      ...
      return false if !@t_discussionmessage_content.nil? && @t_discussionmessage_content !~ Regexp.new(/^[\s\S]{0,65535}$/)
      true
    end
    ...
    def hash
      [fki_discussionmembership_id_actionrequired, t_discussionmessage_content].hash
    end
