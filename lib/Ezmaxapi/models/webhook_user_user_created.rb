...
module EzmaxApi
  ...
  class WebhookUserUserCreated < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::WebhookUserUserCreated` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::WebhookUserUserCreated`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @obj_user.nil?
        invalid_properties.push('invalid value for "obj_user", obj_user cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] obj_user Value to be assigned
    def obj_user=(obj_user)
      if obj_user.nil?
        fail ArgumentError, 'obj_user cannot be nil'
      end
    ...
    def hash
      [obj_webhook, a_obj_attempt, obj_user].hash
    end
