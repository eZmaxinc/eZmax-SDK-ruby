  ...
  class SsprResetPasswordV1Request
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EzmaxApi::SsprResetPasswordV1Request` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EzmaxApi::SsprResetPasswordV1Request`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @s_password.nil?
        invalid_properties.push('invalid value for "s_password", s_password cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @s_password.nil?
      true
    end
    ...
    def hash
      [pks_customer_code, fki_language_id, e_user_type_sspr, s_email_address, s_user_loginname, bin_user_ssp_rtoken, s_password].hash
    end
